...
const Navbar = () => {
  ...
  // Close mobile menu when user scrolls (so it doesn't stay covering the page)
  useEffect(() => {
    const onScroll = () => {
  ...
  return (
    ...
    <nav className='flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all z-50'>
      ...
      {open && (
        // fixed so it overlays when opened; but it now auto-closes on scroll (see effect above)
        <div
          className={`fixed top-[60px] left-0 w-full bg-white shadow-md py-4 flex flex-col items-start gap-2 px-5 text-sm md:hidden z-50`}
        >
          <NavLink
            to='/'
            onClick={() => {
